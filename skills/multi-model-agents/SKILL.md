...
V1.4.5 是 AI Agent Swarm 的当前正式版本，用于把 Opus/Claude 主编码、Gemini 测试分析、Codex 内部审查、可见角色子智能体、MCP 可见进度/日志通知、workspace patch/edit 局部编辑模式和本地项目记忆库（轻量 RAG）纳入 Codex 主控的授权、审查和真实测试流程。
...
7. 默认使用可见角色子智能体工作流。除非任务只是简单问答、纯解释、单条命令查询，或当前线程没有子智能体工具，否则 Coder、Reviewer、Tester、Test Runner、RAG Curator 应以可见 Codex 子智能体方式运行。
...
5. Test Runner Subagent 只运行主控批准的真实命令。
6. RAG Curator Subagent 只整理候选知识，最终写入由 Main Orchestrator 调用 RAG 工具完成。
7. 如果当前线程没有子智能体工具，必须明确说明并降级为 Main Orchestrator 直接调用 MCP 工具完成任务。
