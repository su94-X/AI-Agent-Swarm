...
<p align="center">
  <a href="https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.6"><img alt="Release" src="https://img.shields.io/badge/release-v1.5.6-38BDF8"></a>
...
V1.5.6 吸收了 `gpt55-opus-engineering-loop` 的核心工程规范：标准工程设计/开发计划模板、Official Docs Evidence Gate、Progress Ledger、版本化设计规则、明确的小任务绕过规则和标准 Blocked Report。V1.5.5 起还强化了 `multi_model_coder_workspace_edit` 的坏格式修复路径：当 Opus/Claude 首次返回 markdown、prose、unified diff、partial JSON 或字段不合规的 JSON-like 输出时，插件会触发一次 repair 调用，要求外部 coder 转换为标准 workspace edit JSON 后再校验和应用。

## 两个版本

| 版本 | 适合场景 | 外部模型角色 | Release |
| --- | --- | --- | --- |
| AI Agent Swarm | 完整多模型开发工作流 | Opus/Claude 主编码，Gemini 测试分析，Codex 内部审查 | [v1.5.6](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.6) |
| AI Agent Swarm Lite | 更短、更低成本的审查评分流程 | Codex 自己实现，Opus/Claude 做外部审查和评分，无 Gemini | [v1.5.3-lite.1](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.3-lite.1) |
...
| 写入结果可回传 | `multi_model_coder_workspace_edit` 实际写入后返回脱敏的 `written_files` 内容预览和 readback 校验，方便子智能体把结果交回主控。 |
| 坏格式自动修复 | Opus/Claude 返回 markdown、prose、diff 或 partial JSON 时，插件会做一次 repair 调用，修成标准 workspace edit JSON 后再尝试应用。 |
...
1. 下载发布包：[ai-agent-swarm-1.5.6.zip](https://github.com/su94-X/AI-Agent-Swarm/releases/download/v1.5.6/ai-agent-swarm-1.5.6.zip)
...
- Opus plan-review 有统一输入结构。
- Progress Ledger 有固定位置，方便上下文压缩后恢复。
- 设计版本、计划版本和阻塞历史可以持续追踪。

如果任务依赖第三方框架、SDK、CLI、API、云服务或当前外部事实，再配合使用 [docs/OFFICIAL_DOCS_GATE.md](./docs/OFFICIAL_DOCS_GATE.md)。
...
| [docs/OFFICIAL_DOCS_GATE.md](./docs/OFFICIAL_DOCS_GATE.md) | 官方文档与外部证据闸门 |
| [docs/SUBAGENT_WORKFLOW.md](./docs/SUBAGENT_WORKFLOW.md) | 子智能体流程 |
| [docs/RAG.md](./docs/RAG.md) | 本地项目记忆库 |
| [docs/ROADMAP.md](./docs/ROADMAP.md) | 路线图 |
| [templates/engineering-design.template.md](./templates/engineering-design.template.md) | 工程设计模板 |
| [templates/development-plan.template.md](./templates/development-plan.template.md) | 开发计划模板 |
...
- 主版本 Release：[AI Agent Swarm V1.5.6](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.6)
- Lite Release：[AI Agent Swarm Lite 1.5.3-lite.1](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.3-lite.1)
