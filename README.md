...
<p align="center">
  <a href="https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.4"><img alt="Release" src="https://img.shields.io/badge/release-v1.5.4-38BDF8"></a>
...
V1.5.4 修复了 Coder Subagent 调用 `multi_model_coder_workspace_edit` 后不容易拿到写入内容的问题：工具现在会在实际写入后做 readback hash 校验，并返回受限长度、已脱敏的 `written_files` 内容预览；`dry_run` 会返回 `proposed_files`。V1.5.3 起默认启用模型层流式调用，降低 Opus/Claude 或 Gemini 大上下文、长输出场景下的长时间无响应和网关空闲超时风险；如果某个网关不支持 SSE，可设置 `MMA_MODEL_STREAMING=false` 回退。

## 两个版本

| 版本 | 适合场景 | 外部模型角色 | Release |
| --- | --- | --- | --- |
| AI Agent Swarm | 完整多模型开发工作流 | Opus/Claude 主编码，Gemini 测试分析，Codex 内部审查 | [v1.5.4](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.4) |
...
1. 下载发布包：[ai-agent-swarm-1.5.4.zip](https://github.com/su94-X/AI-Agent-Swarm/releases/download/v1.5.4/ai-agent-swarm-1.5.4.zip)
...
- 主版本 Release：[AI Agent Swarm V1.5.4](https://github.com/su94-X/AI-Agent-Swarm/releases/tag/v1.5.4)
