...
const targetCommitish = process.env.MMA_GITHUB_TARGET_COMMITISH || "codex-only";
const makeLatest = process.env.MMA_GITHUB_MAKE_LATEST || "false";
const outputDir = resolve(process.argv[2] || join(pluginRoot, "..", "..", "outputs"));
const zipPath = resolve(process.argv[3] || join(outputDir, `ai-agent-swarm-codex-${version}.zip`));
const releaseNotePath = join(pluginRoot, "docs", "releases", `GITHUB_RELEASE_CODEX_${version}.md`);
const releaseName = `AI Agent Swarm Codex-only ${version}`;
