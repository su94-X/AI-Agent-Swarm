...
V1.4.7 已把用户提示词入口收敛为三份通用文档：

- `docs/INSTALL_PROMPT.md`：安装、复查、MCP 可见性和离线自检。
- `docs/START_PROMPT.md`：日常开发、新项目、已有项目接手、工程闸门和可见子智能体的统一入口。
- `docs/RELEASE_PROMPT.md`：维护者发布流程，要求分支、tag、GitHub Release、zip asset 和 release 页面核查全部完成。

V1.4.10 已把旧提示词 wrapper 移入 `docs/legacy/`，并新增 `docs/README.md` 作为文档导航。历史 GitHub Release note 已移入 `docs/releases/`。普通用户只需要使用三个入口文档，不需要再从旧提示词里选择。
...
V1.5.3 起外部模型调用默认使用模型层内部 SSE 聚合模式，可通过 `MMA_MODEL_STREAMING=false` 回退到非流式 HTTP JSON 请求：OpenAI-compatible 走 chat completions stream，Anthropic 走 messages stream，Gemini 走 streamGenerateContent。该模式仍会聚合为完整文本后返回给 MCP tool，不改变工具协议，也不会边流边写文件。
