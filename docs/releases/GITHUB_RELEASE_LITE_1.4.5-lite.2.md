# AI Agent Swarm Lite 1.4.5-lite.2 Release Notes
...
- Opus/Claude reviewer/scorer：通过 `multi_model_reviewer_findings` 和 `multi_model_reviewer_score` 提供 plan-review、diff-review、test-review、final-review、证据绑定审查、风险判断和 0-100 评分。
...
- Changed plugin identity to `AI Agent Swarm Lite` with version `1.4.5-lite.2`.
...
- Added engineering gate workflow in `docs/ENGINEERING_GATE.md`.
- Added `review_stage` and `test_evidence` support to reviewer scoring.
...
```text
ai-agent-swarm-lite-1.4.5-lite.2.zip
```
...
1. Download and unzip `ai-agent-swarm-lite-1.4.5-lite.2.zip`.
