...
V1.5.0 起，AI Agent Swarm 随包提供官方 Custom Agent 模板：`.codex/agents/*.toml`。这些模板定义可被 Codex spawn 的角色配置；Skill 负责工作流，MCP 负责外部模型/RAG/workspace 工具，Plugin 负责打包分发。插件安装本身不等于所有项目自动加载子智能体；模板需要位于当前项目 `.codex/agents/` 或用户级 `~/.codex/agents/` 才会被 Codex 当作 Custom Agent 使用。
...
- 如果官方 Custom Agent 模板可用，优先使用 `primary-coder`、`reviewer`、`tester`、`test-runner`、`rag-curator`、`security-auditor`。如果当前客户端只能使用内置 `worker` / `explorer`，则用内置子智能体承载角色，并记录映射关系。
...
- 生命周期：每个子智能体完成后必须关闭，避免已完成线程继续占用并发槽位，导致后续任务无法 spawn 新角色。
...
对非简单任务，Main Orchestrator 必须在开发计划中维护 Progress Ledger。Coder Subagent 每完成一个重要实现步骤后，都要推动主控更新一次，至少包含：
...
- 让多轮对话或上下文压缩后可以快速恢复。
- 让 Reviewer/Test Runner/Tester 明确知道当前已完成到哪一步。
- 避免重复实现已经完成的步骤。

如果上下文被压缩或线程恢复，先从 `docs/engineering/<task-slug>-development-plan.md` 的 Progress Ledger 恢复，再继续执行，不要重新猜上一步。
...
- 如果 development plan 文件在 `allowed_write_paths` 内，重要步骤完成后应同步更新 Progress Ledger。
- 如果不在授权写入范围内，至少把应更新的 Ledger 内容清楚返回给 Main Orchestrator，由主控写回。
- 如果上下文被压缩或线程恢复，先根据 Progress Ledger 判断当前 step，再调用工具。
...
使用 `.codex/agents/security-auditor.toml` 中的角色提示词。

Security Auditor 只做只读安全审计，重点检查 `.env`、API key、GitHub token、RAG 数据、workspace path boundary、symlink 防逃逸、发布包白名单和 prompt injection surface。它不实现代码、不运行破坏性命令、不做最终发布决定；不得打开、读取、打印或转述真实 `.env`、token、凭据、私有日志或 RAG 正文，只能检查 diff、文件名、manifest、发布包条目、`.env.example`、文档说明、脚本逻辑和泄漏路径。

## Blocked Report

只有在安全推进已经不可能或不合理时，子智能体才应输出阻塞报告。常见触发条件：

- MCP 工具不可见或未加载。
- 缺少必要 key、权限或输入证据。
- 当前 step 超出授权路径。
- 多次 repair / retry 后仍无法得到可应用结果。
- 外部服务持续超时或失败，且当前阶段不能安全降级。
...
10. 触碰安全、发布或凭据边界时，Security Auditor 做只读审计。
11. Main Orchestrator 收集子智能体结果后关闭已完成子智能体。
12. Main Orchestrator 写入 RAG 并做最终决策。
