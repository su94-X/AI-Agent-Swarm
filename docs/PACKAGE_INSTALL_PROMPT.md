...
2. 确认 .codex-plugin/plugin.json 是可解析 JSON，并且其中的版本是 1.4.6，展示名是 AI Agent Swarm，开发者是 Su94。
3. 确认包内没有真实 .env 文件。
...
9. 确认 `docs/ENGINEERING_GATE.md` 存在，并提醒我：非简单开发任务正式编码前默认需要设计文档、开发计划和 Opus/Claude plan-review。
10. 如可用，请运行插件校验和以下离线自检：
...
11. 安装完成后，提醒我新开一个 Codex 线程，让 skill 和 MCP tools 重新加载。
...
Codex 应确认包结构完整、manifest 可解析、版本为 `1.4.6`、展示名为 `AI Agent Swarm`、没有 `.env`、没有 `.local/rag`、zip entry 使用 `/` 而不是 Windows 反斜杠、`.mcp.json` 使用相对 MCP 路径，并且工程闸门文档存在。然后完成安装或重新安装，并运行不访问外部模型的 MCP smoke test、HTTP retry self-test、RAG self-test、RAG 子模块 self-test、workspace edit JSON self-test、workspace edit repair self-test 和 tester prompt self-test。
