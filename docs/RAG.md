...
这意味着历史知识不会因为存在于库里就自动进入当前任务上下文。Codex 仍需要按任务筛选少量结果，再传给 Opus/Claude reviewer/scorer。
...
- 用于 plan、constraints、known_test_commands、review context，或传给 Opus/Claude reviewer/scorer 的上下文。
