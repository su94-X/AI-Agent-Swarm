...
workspace edit JSON repair、patch/edit apply repair 或 reviewer/scorer 复审都会额外调用一次外部模型。成本敏感场景先使用更小的上下文、`dry_run`、更窄的 `allowed_read_paths` / `allowed_write_paths`，或在工程计划中明确允许降级。
`MMA_MCP_PROGRESS_NOTIFICATIONS=true` 发送的是客户端可见进度/日志通知，不是带 progress token 的标准进度条。
