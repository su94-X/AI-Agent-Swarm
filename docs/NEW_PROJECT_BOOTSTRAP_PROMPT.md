...
当一个新开发项目开始使用 AI Agent Swarm Lite 插件时，发送下面这段提示词，让 Codex 自动生成项目初始文档、记忆文档和后续本地项目记忆库使用规则。

## 提示词

```text
请使用 AI Agent Swarm Lite 插件为这个新开发项目生成初始项目文档和长期记忆结构。

目标：
1. 建立后续 Codex 主控开发和 Opus/Claude 外部审查评分所需的项目文档。
...
- docs/AGENT_MEMORY.md：给 Codex 和 Opus/Claude 外部审查使用的长期项目记忆摘要。
...
- Reviewer / Scorer 只在需要外部第二意见时调用 Opus/Claude；文档生成和最终判断由 Codex 完成。
- 不使用 Gemini tester 环节；测试策略、真实命令和失败日志判断由 Codex 维护。
